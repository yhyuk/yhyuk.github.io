    ...
    paar: [
      {
        ...
        action: '구독형 B2B SaaS 초기 단계에서 비용 효율이 중요했기 때문에 `Row-level 격리`를 선택했다. 모든 테이블에 tenant_id를 적용하고 쿼리 레벨에서 격리를 강제했으며, 버전 기반 기능 제어로 테넌트별 차등 서비스를 구현했다. 향후 고객 규모가 커지면 Schema-level로 전환 가능한 구조로 설계했다.',
    ...
    paar: [
      {
        ...
        action: '안정성을 최우선 기준으로 삼아 개발 초기부터 `Redis 분산락`을 도입했다. 락 획득 실패 시 지수 백오프로 재시도하여 일시적 충돌을 흡수했고, `Redis Stream + Consumer Group`으로 OpenID별 이벤트 순서를 보장하면서 유실 시 재처리 체계를 구축했다. 충돌이 발생한 후 대응하는 것보다 처음부터 막는 구조를 선택한 결정이었다.',
    ...
    paar: [
      {
        ...
        action: '인프라 변경 없이 즉시 적용 가능한 `ThreadPoolTaskExecutor` 방식을 선택했다. 다만 병렬 처리를 단순히 늘리면 합성 요청이 급증할 때 스레드가 메모리를 과점유할 수 있어, `백프레셔` 슬롯 계산으로 처리 가능한 만큼만 가져오도록 제한했다. 처리 실패 시에는 큐에 되돌리는 방식으로 `at-least-once` 보장을 유지했다.',
    ...
    paar: [
      {
        ...
        action: 'Spring Boot 멀티모듈 아키텍처를 선택해 외부 통신(Connex, Ecommerce 등)을 공통 모듈로 분리했다. 트래픽 규모가 크지 않아 서비스 다운타임을 최소화하면서 DB를 dump하여 직접 이전했고, 데이터 정합성은 이전 전후 수기 검증으로 확보했다. 동시에 API 스펙을 전면 재정의해 불필요한 필드를 제거하고 통합 가능한 엔드포인트를 병합했으며, `Docker 컨테이너` 배포로 전환해 배포 프로세스를 단순화했다.',
    ...
    paar: [
      {
        ...
        action: '분산된 이벤트 어드민 3개를 `Spring Boot 단일 어드민`으로 통합했다. 추첨은 이벤트 특성상 결과를 영구 보존할 필요가 없어 `Redis 기반 확률 추첨`을 선택했고, 중복 참여와 어뷰징 방지 로직을 함께 구현해 추첨 공정성을 확보했다.',
